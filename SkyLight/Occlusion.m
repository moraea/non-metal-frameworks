...
BOOL fake_validateNoOcclusionSinceToken(NSObject* rdi_self, SEL rsi_sel, NSNumber* rdx_token) { return true; }

BOOL fake_isOccluded(NSObject* rdi_self, SEL rsi_sel) { return false; }

void (*real_viewDidMoveToWindow)(NSObject*, SEL);
void fake_viewDidMoveToWindow(NSObject* rdi_self, SEL rsi_sel) {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC), dispatch_get_main_queue(), ^() {
        [NSNotificationCenter.defaultCenter postNotificationName:@"NSOcclusionDetectionViewDidBecomeUnoccluded" object:rdi_self
                                                        userInfo:@ {@"validationToken": SLSecureCursorAssertion.assertion}];
    });
...
void occlusionSetup() {
    ...
    if (appKitAvailable) {
        ...
        swizzleImp(@"NSOcclusionDetectionView", @"viewDidMoveToWindow", true, (IMP)fake_viewDidMoveToWindow,
                   (IMP*)&real_viewDidMoveToWindow);
    } else {
        [NSNotificationCenter.defaultCenter addObserverForName:@"NSApplicationWillFinishLaunchingNotification" object:nil queue:nil
