...
void setHidParams(NSDictionary* params) {
    trace(@"setHidParams IOHIDEventSystemSetProperty %d %@", IOHIDEventSystemSetProperty(eventSystem, CFSTR("HIDParameters"), params),
          params);
}

void hiddSetup() {
    if (isWindowServer) {
        dispatch_queue_attr_t queueSettings =
