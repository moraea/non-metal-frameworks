...
NSString* menuBar2KeyWithDisplay(int display) { return [NSString stringWithFormat:MENUBAR_DARK_FORMAT, display]; }
...
void menuBar2SendCached() {
    ...
    for (NSMutableDictionary* bar in menuBar2ArrayCache) {
        ...
        if (display == -1 || displayCount != 1) {
            ...
            trace(@"MenuBar2 (client): failed finding display for rect %@, count %d, fallback %d, bar: %@", NSStringFromRect(aRect),
                  displayCount, display, bar);
        }
        ...
        if (displayDark) {
            ...
            SLSWindowSetShadowProperties(
                fakeWid, @{
                    @"com.apple.WindowShadowRadiusInactive": @2.5,
                    @"com.apple.WindowShadowDensityInactive": @0.3,
                    @"com.apple.WindowShadowVerticalOffsetInactive": @1.75
                });
        }
        ...
        bar[kCGMenuBarInactiveImageWindowKey] =
            bar[displayDark ? kSLMenuBarInactiveImageWindowDarkKey : kSLMenuBarInactiveImageWindowLightKey];
    }
...
void menuBar2DockRecalculate2() {
    ...
    for (int index = 0; index < CFArrayGetCount(windows); index++) {
        ...
        if (!CGRectEqualToRect(displayRect, rect)) {
            trace(@"MenuBar2 (server): matched display %d, but bounds %@ ≠ wallpaper bounds %@", display, NSStringFromRect(displayRect),
...
void menuBar2DockReduceTransparencyCallback(CFNotificationCenterRef center, void* observer, CFNotificationName name, const void* object,
...
void menuBar2DockAppearanceCallback(CFNotificationCenterRef center, void* observer, CFNotificationName name, const void* object,
                                    CFDictionaryRef userInfo) {
    if (_AXInterfaceGetReduceTransparencyEnabled() || _AXInterfaceGetIncreaseContrastEnabled()) {
        trace(@"MenuBar2 (server): forwarding appearance toggle to clients because Reduce Transparency/Increase Contrast is on");

        [NSDistributedNotificationCenter.defaultCenter postNotificationName:MENUBAR_DARK_NOTE object:nil userInfo:nil
...
void menuBar2UnconditionalSetup() {
    ...
    if ([process containsString:@"Dock.app/Contents/MacOS/Dock"]) {
        [NSNotificationCenter.defaultCenter addObserverForName:@"desktoppicturechanged" object:nil queue:nil usingBlock:^(
                                                                                                                 NSNotification* note) {
        ...
        CFNotificationCenterAddObserver(CFNotificationCenterGetDistributedCenter(), NULL, menuBar2DockReduceTransparencyCallback,
                                        @"AXInterfaceReduceTransparencyStatusDidChange", NULL,
                                        CFNotificationSuspensionBehaviorDeliverImmediately);
        CFNotificationCenterAddObserver(CFNotificationCenterGetDistributedCenter(), NULL, menuBar2DockReduceTransparencyCallback,
                                        @"AXInterfaceIncreaseContrastStatusDidChange", NULL,
                                        CFNotificationSuspensionBehaviorDeliverImmediately);
        CFNotificationCenterAddObserver(CFNotificationCenterGetDistributedCenter(), NULL, menuBar2DockAppearanceCallback,
    ...
    [NSDistributedNotificationCenter.defaultCenter addObserverForName:MENUBAR_DARK_NOTE object:nil queue:nil
